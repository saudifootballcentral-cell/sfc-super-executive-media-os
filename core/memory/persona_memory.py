...
class PersonaMemory:
    """Memory store for a single specialist persona."""
...
class PersonaMemoryStore:
    """Registry of PersonaMemory instances."""

    def __init__(self) -> None:
        self._memories: dict[str, PersonaMemory] = {
            name: PersonaMemory(name) for name in PERSONAS
        }
    ...
    def for_persona(self, persona_name: str) -> PersonaMemory:
        if persona_name not in self._memories:
            ...
            logger.info("[PersonaMemory] Created new persona: %s", persona_name)
