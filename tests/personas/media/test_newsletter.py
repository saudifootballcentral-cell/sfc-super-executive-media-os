...
class TestNewsletterPersona:
    ...
    async def test_health_check(self):
        ...
        assert health["persona_id"] == NewsletterPersona.PERSONA_ID

    async def test_generate_insight(self):
        persona = NewsletterPersona()
        insight = await persona.generate_insight({"task_type": "test", "run_id": "test-run-01"})
        assert insight.persona_id == NewsletterPersona.PERSONA_ID
        assert len(insight.key_recommendations) > 0
        assert insight.persona_name == NewsletterPersona.PERSONA_NAME

    async def test_platform_field(self):
        persona = NewsletterPersona()
        insight = await persona.generate_insight({})
        assert insight.platform == NewsletterPersona.PERSONA_PLATFORM
