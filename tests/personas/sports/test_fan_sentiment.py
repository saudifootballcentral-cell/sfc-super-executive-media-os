...
class TestFanSentimentPersona:
    ...
    async def test_health_check(self):
        ...
        assert health["persona_id"] == FanSentimentPersona.PERSONA_ID

    async def test_generate_insight(self):
        persona = FanSentimentPersona()
        insight = await persona.generate_insight({"task_type": "test", "run_id": "test-run-01"})
        assert insight.persona_id == FanSentimentPersona.PERSONA_ID
